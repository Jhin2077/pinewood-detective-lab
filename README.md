...
- 邮箱注册、登录和退出已经连接真实 Supabase Auth；
- 免费开放阶段关闭注册邮件确认，用户注册后直接登录；配置自定义 SMTP 后再恢复邮件验证；
- 注册必须确认已满 16 周岁并接受社区规则与免责声明，不收集出生日期；
- `#/board` 是无预设卡片的空白案件板编辑器；
- 每个案件板必须选择主类型后才能发布到公共社区；
- 社区支持公开浏览、分类筛选、排行、点赞和评论；
- 访客可拖动公开案件中的线索进行只读调查，但不能修改或保存原作者内容；
- 作者可从公共列表、公开案件页或自己的档案库删除案件，公共版本、互动记录与案件图片会同步清理；
